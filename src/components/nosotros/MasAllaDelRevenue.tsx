...
const MasAllaDelRevenue = ({ section }: { section?: HomeSection }) => {
  ...
  const sectionBg = bgStyle.background ? bgStyle : { background: "linear-gradient(160deg, #0D0D1A 0%, #1A0A2E 40%, #2A1040 70%, #1A0A2E 100%)" };

  return (
    <section className="relative py-24 sm:py-32 px-6 sm:px-10 overflow-hidden" style={sectionBg}>
      {hasBg && <div style={bgLayerStyle} />}
      <div className="absolute pointer-events-none" style={{ width: 600, height: 600, top: -100, left: -200, background: "radial-gradient(circle, rgba(190,24,105,0.12) 0%, transparent 65%)", filter: "blur(120px)" }} />
      <div className="absolute pointer-events-none" style={{ width: 500, height: 500, bottom: -100, right: -150, background: "radial-gradient(circle, rgba(98,36,190,0.15) 0%, transparent 65%)", filter: "blur(120px)" }} />
      ...
      <div className="relative z-10 max-w-[780px] mx-auto text-center">
        <motion.div {...fadeUp(0)} className="flex items-center justify-center gap-4 mb-8">
          ...
          <span className="text-[12px] font-semibold tracking-[0.2em] uppercase" style={{ color: "hsl(var(--teal))", ...getStyle("subtitle") }}>Propósito</span>
          <div className="h-px w-10" style={{ background: "rgba(255,255,255,0.2)" }} />
        </motion.div>
        ...
        <div className="mt-10 space-y-6">
          {[p1, p2, p3].map((p, i) => (
            <motion.p key={i} {...fadeUp(0.14 + i * 0.06)} className="text-[17px] sm:text-[19px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.6)", ...getStyle("body") }}>
              {p}
            </motion.p>
          ))}
        </div>

        <motion.div {...fadeUp(0.38)} className="mt-16 sm:mt-20 pt-10 border-t" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
          <p className="text-[26px] sm:text-[34px] md:text-[40px] font-bold leading-[1.15] tracking-tight" style={{ background: "linear-gradient(135deg, hsl(var(--teal)), hsl(var(--blue)))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            {closingQuote}
          </p>
        </motion.div>
      </div>
