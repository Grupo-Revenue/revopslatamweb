        ...
        <motion.div {...fadeUp(0.25)} className="mt-10 flex flex-col items-center gap-4">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 sm:gap-4 w-full sm:w-auto">
            {(meta.cta_style_key as string) ? (
              <DynamicCTA styleKey={meta.cta_style_key as string} onClick={() => meta.cta1_opens_lead_form ? openLeadForm("final-cta1") : section?.cta_url ? window.open(section.cta_url, "_blank") : openLeadForm("final-cta")}>
                {cta}
              </DynamicCTA>
            ) : (
              <Button size="lg" className="gap-2" onClick={() => meta.cta1_opens_lead_form ? openLeadForm("final-cta1") : section?.cta_url ? window.open(section.cta_url, "_blank") : openLeadForm("final-cta")}>
                {cta}
                <ArrowRight size={18} />
              </Button>
            )}
            {(meta.cta2_style_key as string) ? (
              <DynamicCTA styleKey={meta.cta2_style_key as string} onClick={() => meta.cta2_opens_lead_form ? openLeadForm("final-cta2") : window.open((meta.cta2_url as string) || "https://pulso.revopslatam.com/", "_blank")}>
                {cta2}
              </DynamicCTA>
            ) : (
              <Button size="lg" variant="outline" className="border-2 border-[rgba(255,255,255,0.3)] text-primary-foreground hover:border-primary-foreground bg-transparent" onClick={() => meta.cta2_opens_lead_form ? openLeadForm("final-cta2") : window.open((meta.cta2_url as string) || "https://pulso.revopslatam.com/", "_blank")}>
                {cta2}
              </Button>
            )}
          </div>
