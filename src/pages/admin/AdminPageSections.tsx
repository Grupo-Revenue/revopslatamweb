                ...
                <div className="px-4 pb-4 border-t border-zinc-800 pt-4 space-y-4">
                  ...
                  <div className="p-3 bg-zinc-800/50 rounded-xl border border-zinc-700/50 space-y-3">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                      ...
                      <div>
                        <Label className="text-zinc-500 text-[10px] uppercase tracking-wider">📱 Tablet</Label>
                        <Input
                          value={(meta.image_max_width_tablet as string) ?? ""}
                          onChange={(e) => updateSectionLocal(section.id, "metadata", { ...meta, image_max_width_tablet: e.target.value || undefined })}
                          className="bg-zinc-800 border-zinc-700 text-white mt-1 text-sm"
                          placeholder="Ej: 80%"
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 p-3 bg-zinc-800/50 rounded-xl border border-zinc-700/50">
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        <Input
                          ...
                          placeholder="Ej: #BE1869, linear-gradient(...)"
                        />
                        <Button
                          type="button"
                          size="sm"
                          variant={meta.badge_bg === "" ? "default" : "outline"}
                          className="text-[10px] h-9 px-3 whitespace-nowrap"
                          onClick={() => updateSectionLocal(section.id, "metadata", { ...meta, badge_bg: "" })}
                        >
                          Sin fondo
                        </Button>
                      </div>
                      <p className="text-zinc-600 text-[10px] mt-0.5">Fondo del badge. Acepta color sólido o gradiente CSS. "Sin fondo" = transparente.</p>
                    </div>
                    <div>
                      <Label className="text-zinc-500 text-[10px] uppercase tracking-wider">Color del texto del Badge</Label>
                      <div className="flex items-center gap-2 mt-1">
                        <Input
                          value={(meta.badge_color as string) ?? ""}
                          onChange={(e) => updateSectionLocal(section.id, "metadata", { ...meta, badge_color: e.target.value || undefined })}
                          className="bg-zinc-800 border-zinc-700 text-white text-sm flex-1"
                          placeholder="Ej: #ffffff"
                        />
                        <input
                          type="color"
                          value={(meta.badge_color as string) || "#ffffff"}
                          onChange={(e) => updateSectionLocal(section.id, "metadata", { ...meta, badge_color: e.target.value })}
                          className="h-8 w-8 rounded border border-zinc-700 cursor-pointer bg-transparent"
                        />
                      </div>
                    </div>
                  </div>
