  ...
  return (
    <div ref={ref} className="relative max-w-[600px] mx-auto pl-10 md:pl-14">
      ...
      <svg className="absolute left-[17px] md:left-[25px] top-0 w-[2px] h-full">
        <defs><linearGradient id="phLine" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#BE1869" /><stop offset="100%" stopColor="#6224BE" /></linearGradient></defs>
        <motion.line x1="1" y1="0" x2="1" y2="100%" stroke="url(#phLine)" strokeWidth="2" strokeDasharray="500" initial={{ strokeDashoffset: 500 }} animate={inView ? { strokeDashoffset: 0 } : {}} transition={{ duration: 1.8 }} />
      </svg>
      ...
      <SectionShell section={hero} className="min-h-[90vh] flex items-center" defaultBg={{ background: "#1A1A2E" }}>
        ...
        <div className="relative z-10 w-full max-w-[1100px] mx-auto px-6 pt-32 pb-20 grid lg:grid-cols-[55%_45%] gap-12 items-center">
          <div>
            ...
            <motion.span {...fadeUp(0.05)} className="inline-block text-[11px] uppercase font-bold tracking-wider px-4 py-1.5 rounded-full mb-6 text-white" style={{ background: (hm.badge_bg as string) ? (hm.badge_bg as string) : hm.badge_bg === "" ? "transparent" : GRADIENT, color: (hm.badge_color as string) || "#fff" }}>{h.badge}</motion.span>
            ...
            <motion.div {...fadeUp(0.2)} className="flex flex-wrap items-center gap-4">
              <button className="text-sm font-semibold text-white rounded-full px-8 py-3.5 transition-all duration-200 hover:scale-[1.03] hover:shadow-[0_0_32px_rgba(190,24,105,0.4)]" style={{ background: (hm.cta_bg as string) || GRADIENT }}>{h.cta}</button>
