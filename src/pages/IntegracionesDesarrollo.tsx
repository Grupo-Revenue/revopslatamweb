...
/* ─── Ecosystem Diagram ─── */
function EcosystemDiagram({ nodes }: { nodes: typeof DEF.hero.nodes }) {
  ...
  if (isMobile) {
    return (
      <div ref={ref} className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="flex items-center justify-center gap-3 mb-6">
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-sm font-extrabold text-white" style={{ background: HUBSPOT }}>HS</div>
          <span className="text-xs font-bold text-white/60 uppercase tracking-wider">HubSpot CRM</span>
        </div>
        <div className="space-y-2">
          {nodes.map((n, i) => (
            <motion.div key={i} className="flex items-center gap-3 rounded-xl px-4 py-3"
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}
              initial={{ opacity: 0, x: -16 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.2 + i * 0.1 }}>
              <div className="w-2 h-2 rounded-full shrink-0" style={{ background: GRADIENT }} />
              <div>
                <span className="text-xs font-bold text-white">{n.label}</span>
                {n.sub && <span className="text-[10px] text-white/40 ml-1">{n.sub}</span>}
                <span className="block text-[10px] text-white/40">{n.data}</span>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    <div ref={ref} className="rounded-2xl p-10 relative" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)" }}>
      <svg viewBox="0 0 280 280" className="w-full max-w-[280px] mx-auto">
          ...
          return (
            <g key={`line-${i}`}>
              <line x1={cx} y1={cy} x2={nx} y2={ny}
                stroke="url(#lineGrad)" strokeWidth={isHovered ? 2 : 1}
                opacity={isHovered ? 1 : 0.3}
                className="transition-all duration-300" />
              {/* Particle */}
              <circle r={isHovered ? 3 : 2} fill="url(#lineGrad)" opacity={0.8}>
                <animateMotion dur={`${2 + i * 0.3}s`} repeatCount="indefinite"
                  path={`M${cx},${cy} L${nx},${ny}`} />
              </circle>
              <circle r={isHovered ? 2.5 : 1.5} fill="url(#lineGrad)" opacity={0.5}>
                <animateMotion dur={`${2 + i * 0.3}s`} begin={`${1 + i * 0.15}s`} repeatCount="indefinite"
        ...
        <text x={cx} y={cy + 5} textAnchor="middle" fill="white" fontSize="13" fontWeight="800">HS</text>
        ...
        {nodes.map((n, i) => {
          ...
          return (
            ...
            >
              <circle cx={nx} cy={ny} r={20} fill="rgba(255,255,255,0.06)" stroke={isHovered ? HUBSPOT : "rgba(255,255,255,0.15)"} strokeWidth={isHovered ? 1.5 : 1} className="transition-all duration-300" />
              <text x={nx} y={ny + 1} textAnchor="middle" fill="white" fontSize="7" fontWeight="700">{n.label}</text>
              <text x={nx} y={ny + 9} textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize="5">{n.sub}</text>
            </g>
          );
        })}
      </svg>
      ...
      <AnimatePresence>
        {hoveredNode !== null && (
          <motion.div
            ...
            className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[11px] px-4 py-2 rounded-full whitespace-nowrap"
            style={{ background: "#1A1A2E", color: "#fff", border: `1px solid ${HUBSPOT}` }}>
            <span style={{ color: HUBSPOT }}>{nodes[hoveredNode].label}:</span> {nodes[hoveredNode].data}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
